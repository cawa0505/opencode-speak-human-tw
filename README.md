...
> AI 寫作工作坊開放報名了。三小時，教三件事：怎麼讓 AI 抓到你的語氣、怎麼建自己的改稿流程、怎麼把重複的產文工作自動化。上一梯 40 個名額三天滿，這梯加開到 60 個。
...
> 開信率從 31% 掉到 24%。讀者的習慣比我想像的頑固：你們就是習慣週五早上配咖啡看信。好，我學到了，下期改回週五。
...
4. **逐類改寫**：31 種痕跡逐類處理，模式優先、詞表兜底；台灣用語檢查同步跑
...
| 語言句式類 | 7 | 「不是 A 而是 B」堆疊、排比三段式、解說導引句、勸誡反問收尾 |
| 風格版面類 | 8 | 破折號過密、粗體轟炸、emoji 堆疊、編號切碎段落、表格誤用 |
| 溝通殘留類 | 6 | 「希望這對你有幫助」、諂媚開場、知識截止免責、模板佔位文字 |
...
- **語言**：從頭以繁體中文與台灣用語校準，內建中國用語滲透檢查。不是簡轉繁
...
核心只需要 `SKILL.md` 一個檔案（lite）；要完整的 31 種痕跡、台灣用語表和誤殺防護，帶上 `references/` 完整包（full）。
